package permoveo.com.bluetoothdetector.util;

/**
 * Created by byfieldj on 4/16/17.
 */
...
public class Bluetooth {
    ...
    public boolean connect(final String deviceAddress) {
        ...
        if (mBluetoothGatt != null && mBluetoothGatt.getDevice().getAddress().equals(deviceAddress)) {
            // just reconnect
            Log.d("BluetoothTracker", "Connecting to previous device");
            return mBluetoothGatt.connect();
        } else {
            Log.d("BluetoothTracker", "Connecting from scratch");
    ...
    public void startServicesDiscovery() {
        if (mBluetoothGatt != null)
            ...
            Log.d("BluetoothTracker", "Bluetooth gatt is NULL");
    }
    ...
    public void getSupportedServices() {
        Log.d("BluetoothTracker", "getSupportedServices");
        if (mBluetoothGattServices != null && mBluetoothGattServices.size() > 0) {
            mBluetoothGattServices.clear();
            Log.d("Bluetooth", "Gatt services size -> " + mBluetoothGattServices.size());
        }
    ...
    public void getCharacteristicValue(BluetoothGattCharacteristic ch) {
        ...
        if (uuid.toString().equals(BluetoothConstants.BT_CHAR_MODEL_NUMBER) || // model number string
        		 uuid.toString().equals(BluetoothConstants.BT_CHAR_SERIAL_NUMBER )|| // serial number string)
        		 uuid.toString().equals(BluetoothConstants.BT_CHAR_DEVICE_NAME) || uuid.toString().equals(BluetoothConstants.BT_CHAR_MANUFACTURER_NAME) ) // firmware revision string
        {
        ...
        else if(uuid.toString().equals(BluetoothConstants.BT_CHAR_BATTERY_LEVEL)) { // battery level
    ...
    private final BluetoothGattCallback mBleCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                //mBluetoothGatt.readRemoteRssi();
                // response wistartServicesDiscoveryll be delivered to callback object!
            ...
            Log.d("BluetoothTracker", "Status -> " + status + ", state -> " + newState);

        }

        @Override
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            Log.d("BluetoothTracker", "OnServicesDiscovered");
    ...
    private void processTxQueue() {
        ...
        Log.d("BluetoothTracker", "Current queue size -> " + txQueue.size());
